...
const average = (values) => values.length
    ...
    : 0;
const toOperationCategory = (type) => {
    ...
    if (normalized.includes("return"))
        return "return";
    if (normalized === "warehouse_transfer")
        return "transfer";
    if (normalized === "after_sell_discount")
        return "discount";
    return "other";
};
const getTimestampFromFields = (row, fields) => {
    for (const field of fields) {
        const rawValue = row[field];
        if (rawValue == null) {
            continue;
        }
        const parsed = field === "createdAt" && typeof rawValue === "number"
            ? toNumber(rawValue)
...
const pickTodayRows = (rows, dateFields, todayDateKey, timezone) => {
...
const readCollection = (path) => __awaiter(void 0, void 0, void 0, function* () {
...
const buildTodayOverview = (...args_1) => __awaiter(void 0, [...args_1], void 0, function* (options = {}) {
    ...
    const [operationCollections, customersRaw, suppliersRaw] = yield Promise.all([
        (0, operations_service_1.getOperationCollections)(),
        readCollection("customer"),
        readCollection("supplier"),
    ]);
    const operations = (0, operations_service_1.normalizeOperations)(operationCollections.purchases, operationCollections.sells, operationCollections.payments, operationCollections.returns, operationCollections.transfers, operationCollections.discounts);
    let invalidOperationDates = 0;
    const operationsToday = operations.filter((operation) => {
        const timestamp = (0, operations_service_1.toTimestamp)(operation.date);
        if (timestamp <= 0) {
    ...
    const purchasesSlice = pickTodayRows(operationCollections.purchases, ["date"], todayDateKey, timezone);
    const sellsSlice = pickTodayRows(operationCollections.sells, ["date"], todayDateKey, timezone);
    const paymentsSlice = pickTodayRows(operationCollections.payments, ["date"], todayDateKey, timezone);
    const returnsSlice = pickTodayRows(operationCollections.returns, ["createdDate", "date"], todayDateKey, timezone);
    const transfersSlice = pickTodayRows(operationCollections.transfers, ["createdAt", "date"], todayDateKey, timezone);
    const discountsSlice = pickTodayRows(operationCollections.discounts, ["date"], todayDateKey, timezone);
    ...
    const invalidDateRecords = invalidOperationDates +
    ...
    const transferCostToday = transfersToday.reduce((sum, transfer) => { var _a; return sum + toNumber((_a = transfer.cost) !== null && _a !== void 0 ? _a : transfer.amount); }, 0);
    const returnEstimatedValue = returnsToday.reduce((sum, row) => sum + toNumber(row.returnValue), 0);
    ...
    const customerAttentionRows = customersRaw
    ...
    const supplierAttentionRows = suppliersRaw
        .map((entry) => asRecord(entry))
        .map((supplier) => {
        ...
        .sort((a, b) => b.openAmount - a.openAmount);
    const openReceivablesTotal = customerAttentionRows.reduce((sum, row) => sum + row.openAmount, 0);
    const openPayablesTotal = supplierAttentionRows.reduce((sum, row) => sum + row.openAmount, 0);
    ...
    const previousDateKeys = Array.from({ length: BASELINE_DAYS }, (_, index) => toDateKey(Date.now() - (index + 1) * DAY_MS, timezone));
    ...
    const highlights = [];
    if (operationsToday.length > 0) {
        highlights.push(`${operationsToday.length} operations were recorded today across ${warehouseCounts.size} active warehouses.`);
